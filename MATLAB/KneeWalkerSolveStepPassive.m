function [ t,X,steptracker,lockflags,offset,Xend,Xnew ] = KneeWalkerSolveStepPassive(maxsteps,X0,tstep,tspan,R,gval,gamval,onelockdat,twolockdat,Text)
...
%   Detailed explanation goes here
    ...
    onelockdynam = @(t,X) ChainDynamics(t,X,onelockdat.Mf,onelockdat.Cf,onelockdat.Tf,Text,gval,gamval);
    onelockcollideevent = @(t,X) CheckCollision(t,X,onelockdat.Xf,onelockdat.numlinks,gamval,R,RotMat,3,2,true);
    ...
    twolockcollideevent = @(t,X) CheckCollision(t,X,twolockdat.Xf,twolockdat.numlinks,gamval,R,RotMat,3,2,false); %knee already locked, don't check knee
    ...
    while(numsteps < maxsteps)
        ...
        if (ie == 2) %knee lock event
            ...
            X1 = repelem(X1,1,[2 1 1 2 1 1]); %each pair of joints shares position and velocity
        ...
        elseif (ie == 1) %foot strike event
            ...
            if (pastevent == 2) %foot strike after knee strike
                X1 = repelem(X1,1,[2 2 2 2]); %each pair of joints shares position and velocity                
                X = [X;[X1(1:end-1,:),zeros(size(X1,1)-1,size(X,2)-size(X1,2))]]; %update data
